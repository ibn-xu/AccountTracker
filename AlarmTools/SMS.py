...
from .AlarmBase import BaseAlarm
from .AlarmSettings import SMS_setting
...
class SMSAlarm(BaseAlarm):
    ...
    def send(self, msg: str='',template:list = ['预警测试','超出回撤区间50%']):
        ...
        try:
            ...
            req.SmsSdkAppid = SMS_setting['req_Appid']
            # 短信签名内容: 使用 UTF-8 编码，必须填写已审核通过的签名，可登录 [短信控制台] 查看签名信息
            req.Sign = SMS_setting['req_Sign']
            # 短信码号扩展号: 默认未开通，如需开通请联系 [sms helper]
            req.ExtendCode = ""
            # 用户的 session 内容: 可以携带用户侧 ID 等上下文信息，server 会原样返回
            req.SessionContext = SMS_setting['req_SessionContext']
            # 国际/港澳台短信 senderid: 国内短信填空，默认未开通，如需开通请联系 [sms helper]
            req.SenderId = ""
            # 下发手机号码，采用 e.164 标准，+[国家或地区码][手机号]
            # 例如+8613711112222， 其中前面有一个+号 ，86为国家码，13711112222为手机号，最多不要超过200个手机号
            req.PhoneNumberSet = SMS_setting['req_NumberSet']
            # 模板 ID: 必须填写已审核通过的模板 ID，可登录 [短信控制台] 查看模板 ID
            req.TemplateID = SMS_setting['req_TemplateID']
            # 模板参数: 若无模板参数，则设置为空
